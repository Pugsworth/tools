...
<template>
  ...
  <div id="app" ref="appContainer" class="p-2 w-full h-screen flex flex-col gap-2 relative" @paste="handlePaste"
      ...
      <div
        ...
        <div v-show="uiState.showPalette" class="p-3 flex gap-3">
          ...
          <div class="w-64 shrink-0 flex flex-col gap-2">
            <!-- Segmentation -->
            <div class="flex items-center gap-1 bg-slate-900/50 p-1.5 rounded-lg border border-slate-700/50">
              ...
              <span v-if="isComputingSeg" class="ml-auto text-[9px] text-blue-400 animate-pulse">...</span>
            </div>
              ...
              <div class="flex flex-wrap gap-2 items-center">
                ...
                <div class="flex gap-0.5 ml-auto sm:ml-0">
                  <button @click="sortPalette('center')"
                    class="px-1.5 py-0.5 bg-slate-700 hover:bg-slate-600 rounded text-[9px]" title="Distance from Grey">
                    <Target class="w-3 h-3" />
                  </button>
                  <button @click="sortPalette('mean')"
                    class="px-1.5 py-0.5 bg-slate-700 hover:bg-slate-600 rounded text-[9px]" title="Average Intensity">
    ...
    <div class="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden flex flex-col shrink-0 transition-all">
        ...
        <div class="flex-grow p-2 overflow-y-auto palette-scroll relative select-none bg-slate-800/50">
          <div v-if="activePalette.length" class="flex flex-wrap gap-1.5">
            <div v-for="(color, index) in activePalette" :key="color.id"
              ...
              <div
                class="color-box w-8 h-8 md:w-10 md:h-10 rounded border border-slate-600 shadow-sm transition-all relative ring-offset-2 ring-offset-slate-800"
                :class="[
                ...
                <div
                  class="absolute -top-2 -right-2 bg-slate-900 rounded-full p-0.5 shadow-md cursor-pointer transition-opacity z-20"
          ...
          <div class="flex-grow overflow-y-auto p-2 space-y-2" v-if="selectedSwatchId">
            ...
            <div class="flex gap-2 h-24">
              <div ref="satValBox"
                ...
                <canvas v-if="isLabMode" ref="visualCanvas" class="absolute inset-0 w-full h-full" width="64"
                  height="64"></canvas>
              ...
              <div class="w-4 rounded-full cursor-pointer overflow-hidden border border-slate-600 relative"
                ...
                <div
                  class="absolute left-0 right-0 h-1 bg-white border border-slate-400 shadow -mt-0.5 pointer-events-none"
            ...
            <div class="space-y-1.5">
              <div v-for="(val, key) in currentModeValues" :key="key" class="flex items-center gap-1.5">
                ...
                <input type="range" :min="getMin(key)" :max="getMax(key)" :step="getStep(key)" :value="val"
                ...
                <input type="number" :value="Math.round(val)" @input="updateColorFromSlider(key, $event.target.value)"
    ...
    <div class="relative flex-grow bg-slate-950 rounded-lg border border-slate-800 overflow-hidden shadow-xl min-h-0">
      <!-- Controls -->
      ...
      <div ref="containerRef" class="canvas-container w-full h-full relative" @wheel.prevent="handleWheel"
        ...
        <div ref="transformContainer" class="absolute top-0 left-0 origin-top-left" :style="transformStyle">
          ...
          <div v-if="isSplitView"
            ...
            :style="{ left: splitPercent + '%', width: '40px', marginLeft: '-20px' }" @mousedown.stop="startSplitDrag">
            <div
